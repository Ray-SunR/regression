...
class Document(Base):
	...
	def bson(self, collections, refversion, tarversion):
		...
		for benchmark in self.get('benchmarks'):
			...
			id = benchmark.bson(collections, refversion, tarversion)
...
# A run represents a benchmark output
class Run(Base):
	'''
	...
	def __init__(self):
		self._impl = {
			...
			'hash': '', # document hash
			'pages': [], # list(Page_id)
			'diffs': {} # map from version -> list of diff_ids
		}
...
class Benchmark(Base):
	...
	def populate(self, regression, document, run):
		...
		for file in regression.RefOutFilePaths():
			...
			diff_page_path = regression.DiffMetricsTarMap()[file]
